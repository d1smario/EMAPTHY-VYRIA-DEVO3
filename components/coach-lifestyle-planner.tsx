...
const DAYS = [
  { id: "monday", label: "Lunedi" },
  { id: "tuesday", label: "Martedi" },
  { id: "wednesday", label: "Mercoledi" },
  { id: "thursday", label: "Giovedi" },
  { id: "friday", label: "Venerdi" },
...
export function CoachLifestylePlanner({ coachId, linkedAthletes }: CoachLifestylePlannerProps) {
  ...
  const handleSaveSession = async () => {
    ...
    try {
      ...
      // Get existing workouts for the athlete
      ...
      if (existingData) {
        ...
        const existingIntervals = existingData.intervals || {}
        const lifestyleSessions = existingIntervals.lifestyleSessions || []
        lifestyleSessions.push(sessionData)
        ...
        // Create new entry
        const { error: insertError } = await supabase.from("weekly_workouts").insert({
          user_id: selectedAthlete,
          day_of_week: selectedDay,
          workout_type: "lifestyle",
          intervals: {
            lifestyleSessions: [sessionData],
          },
        })
      ...
      alert(`Sessione assegnata a ${athlete.athlete?.user.full_name || "atleta"} per ${DAYS.find((d) => d.id === selectedDay)?.label}`)
